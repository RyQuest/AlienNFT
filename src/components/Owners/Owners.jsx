import React from "react";

export default function Owners() {
  return (
    <section
      className="u-align-center u-clearfix u-custom-color-3 u-section-6"
      id="sec-e67a"
    >
      <div className="u-clearfix u-sheet u-valign-middle-md u-sheet-1">
        <h1 className="u-custom-font u-text u-text-font u-text-1">
          {" "}
          Meet the Team
        </h1>
        <div className="u-expanded-width u-list u-list-1">
          <div className="u-repeater u-repeater-1">
            <div className="u-container-style u-list-item u-repeater-item">
              <div className="u-container-layout u-similar-container u-container-layout-1">
                <h2 className="u-align-justify u-custom-font u-text u-text-font u-text-2">
                  Womens NFT is the latest art project by the artist, Maliha
                  Abidi. She is a Pakistani-American published author and visual
                  artist. Her work focuses on advocating for social justice
                  including women’s rights, gender equality, girl’s education
                  and fighting for the marginalised communities. Maliha has been
                  sharing st​ories of incredible women through her art and
                  illustrated books for over 9 years. She has worked with
                  amazing organisations such as UN, Global Goals, Malala Fund,
                  Adobe, Google, and many others. Her work has featured on
                  various media platforms such as The New York Times, Good
                  Morning America, BBC, TRT World and more. Maliha is excited to
                  bring the amazing 'rising women' to the NFT world. Maliha
                  Abidi’s 2nd Book, RISE, was launched on the 4th of November
                  2021. Special book give-aways are planned for Womens NFT
                  collectors. Check out her social media for past, present and
                  future projects!&nbsp;
                  <br />
                  <br />
                  Fun fact about Maliha: She can go on for days just on coffee
                  and noodles... daaaaays. true story!
                  <br />
                </h2>
                <img
                  className="u-image u-image-round u-radius-17 u-image-1"
                  alt=""
                  data-image-width={920}
                  data-image-height={810}
                  src="https://womenrise.art/images/01.jpg"
                />
                <h3 className="u-align-center u-custom-font u-text u-text-font u-text-3">
                  Maliha Abidi
                </h3>
                <h3 className="u-align-center u-custom-font u-text u-text-font u-text-4">
                  {" "}
                  Artist and Founder
                </h3>
                <div className="u-social-icons u-spacing-10 u-social-icons-1">
                  <a
                    className="u-social-url"
                    href="https://twitter.com/Maliha_z_Art"
                    title="Twitter"
                    target="_blank"
                  >
                    <span className="u-icon u-icon-circle u-social-icon u-social-twitter u-text-grey-5 u-icon-1">
                      <svg
                        className="u-svg-link"
                        preserveAspectRatio="xMidYMin slice"
                        viewBox="0 0 112 112"
                        style={{}}
                      >
                        <use
                          xmlnsXlink="http://www.w3.org/1999/xlink"
                          xlinkHref="#svg-bf59"
                        ></use>
                      </svg>
                      <svg
                        className="u-svg-content"
                        viewBox="0 0 112 112"
                        x="0px"
                        y="0px"
                        id="svg-bf59"
                      >
                        <path d="M92.2,38.2c0,0.8,0,1.6,0,2.3c0,24.3-18.6,52.4-52.6,52.4c-10.6,0.1-20.2-2.9-28.5-8.2 c1.4,0.2,2.9,0.2,4.4,0.2c8.7,0,16.7-2.9,23-7.9c-8.1-0.2-14.9-5.5-17.3-12.8c1.1,0.2,2.4,0.2,3.4,0.2c1.6,0,3.3-0.2,4.8-0.7 c-8.4-1.6-14.9-9.2-14.9-18c0-0.2,0-0.2,0-0.2c2.5,1.4,5.4,2.2,8.4,2.3c-5-3.3-8.3-8.9-8.3-15.4c0-3.4,1-6.5,2.5-9.2 c9.1,11.1,22.7,18.5,38,19.2c-0.2-1.4-0.4-2.8-0.4-4.3c0.1-10,8.3-18.2,18.5-18.2c5.4,0,10.1,2.2,13.5,5.7c4.3-0.8,8.1-2.3,11.7-4.5 c-1.4,4.3-4.3,7.9-8.1,10.1c3.7-0.4,7.3-1.4,10.6-2.9C98.9,32.3,95.7,35.5,92.2,38.2z"></path>
                      </svg>
                    </span>
                  </a>
                  <a
                    className="u-social-url"
                    target="_blank"
                    href="https://www.instagram.com/maliha_z_art/"
                    title="Insta"
                  >
                    <span className="u-icon u-icon-circle u-social-icon u-social-instagram u-text-grey-5 u-icon-2">
                      <svg
                        className="u-svg-link"
                        preserveAspectRatio="xMidYMin slice"
                        viewBox="0 0 112 112"
                        style={{}}
                      >
                        <use
                          xmlnsXlink="http://www.w3.org/1999/xlink"
                          xlinkHref="#svg-c683"
                        ></use>
                      </svg>
                      <svg
                        className="u-svg-content"
                        viewBox="0 0 112 112"
                        x="0px"
                        y="0px"
                        id="svg-c683"
                      >
                        <path d="M55.9,32.9c-12.8,0-23.2,10.4-23.2,23.2s10.4,23.2,23.2,23.2s23.2-10.4,23.2-23.2S68.7,32.9,55.9,32.9z M55.9,69.4c-7.4,0-13.3-6-13.3-13.3c-0.1-7.4,6-13.3,13.3-13.3s13.3,6,13.3,13.3C69.3,63.5,63.3,69.4,55.9,69.4z"></path>
                        <path d="M79.7,26.8c-3,0-5.4,2.5-5.4,5.4s2.5,5.4,5.4,5.4c3,0,5.4-2.5,5.4-5.4S82.7,26.8,79.7,26.8z"></path>
                        <path d="M78.2,11H33.5C21,11,10.8,21.3,10.8,33.7v44.7c0,12.6,10.2,22.8,22.7,22.8h44.7c12.6,0,22.7-10.2,22.7-22.7 V33.7C100.8,21.1,90.6,11,78.2,11z M91,78.4c0,7.1-5.8,12.8-12.8,12.8H33.5c-7.1,0-12.8-5.8-12.8-12.8V33.7 c0-7.1,5.8-12.8,12.8-12.8h44.7c7.1,0,12.8,5.8,12.8,12.8V78.4z"></path>
                      </svg>
                    </span>
                  </a>
                  <a
                    className="u-social-url"
                    target="_blank"
                    title="Tiktok"
                    href="https://www.tiktok.com/@malihazart"
                  >
                    <span className="u-icon u-social-custom u-social-icon u-text-grey-5 u-icon-3">
                      <svg
                        className="u-svg-link"
                        preserveAspectRatio="xMidYMin slice"
                        viewBox="0 0 512 512"
                        style={{}}
                      >
                        <use
                          xmlnsXlink="http://www.w3.org/1999/xlink"
                          xlinkHref="#svg-bede"
                        ></use>
                      </svg>
                      <svg
                        className="u-svg-content"
                        viewBox="0 0 512 512"
                        id="svg-bede"
                      >
                        <path d="m256 0c-141.363 0-256 114.637-256 256s114.637 256 256 256 256-114.637 256-256-114.637-256-256-256zm128.43 195.873v34.663c-16.345.006-32.226-3.197-47.204-9.516-9.631-4.066-18.604-9.305-26.811-15.636l.246 106.693c-.103 24.025-9.608 46.598-26.811 63.601-14 13.84-31.74 22.641-50.968 25.49-4.518.669-9.116 1.012-13.766 1.012-20.583 0-40.124-6.668-56.109-18.97-3.008-2.316-5.885-4.827-8.624-7.532-18.644-18.427-28.258-43.401-26.639-69.674 1.235-19.999 9.242-39.072 22.59-54.021 17.66-19.782 42.366-30.762 68.782-30.762 4.65 0 9.248.349 13.766 1.018v12.816 35.652c-4.284-1.413-8.859-2.19-13.623-2.19-24.134 0-43.659 19.69-43.298 43.842.229 15.453 8.67 28.961 21.12 36.407 5.851 3.5 12.582 5.668 19.765 6.062 5.628.309 11.032-.475 16.036-2.127 17.243-5.696 29.682-21.892 29.682-40.994l.057-71.447v-130.44h47.736c.046 4.73.526 9.345 1.418 13.817 3.603 18.101 13.806 33.805 28.006 44.511 12.382 9.339 27.8 14.875 44.511 14.875.011 0 .149 0 .137-.011v12.861z"></path>
                      </svg>
                    </span>
                  </a>
                  <a
                    className="u-social-url"
                    target="_blank"
                    title="Website"
                    href="https://www.malihaabidi.com"
                  >
                    <span className="u-icon u-social-custom u-social-icon u-text-grey-5 u-icon-4">
                      <svg
                        className="u-svg-link"
                        preserveAspectRatio="xMidYMin slice"
                        viewBox="0 0 511.999 511.999"
                        style={{}}
                      >
                        <use
                          xmlnsXlink="http://www.w3.org/1999/xlink"
                          xlinkHref="#svg-1182"
                        ></use>
                      </svg>
                      <svg
                        className="u-svg-content"
                        viewBox="0 0 511.999 511.999"
                        x="0px"
                        y="0px"
                        id="svg-1182"
                        style={{ enableBackground: "new 0 0 511.999 511.999" }}
                      >
                        <g>
                          <g>
                            <path d="M437.019,74.981C388.667,26.628,324.38,0,256,0C187.62,0,123.332,26.628,74.981,74.98C26.628,123.332,0,187.62,0,256    s26.628,132.667,74.981,181.019C123.332,485.371,187.62,511.999,256,511.999c68.381,0,132.667-26.628,181.02-74.981    c48.351-48.351,74.98-112.639,74.98-181.019S485.371,123.332,437.019,74.981z M96.216,96.216    c22.511-22.511,48.938-39.681,77.742-50.888c-7.672,9.578-14.851,20.587-21.43,32.969c-7.641,14.38-14.234,30.173-19.725,47.042    c-19.022-3.157-36.647-7.039-52.393-11.595C85.345,107.678,90.61,101.822,96.216,96.216z M62.229,139.585    c18.417,5.897,39.479,10.87,62.461,14.809c-6.4,27.166-10.167,56.399-11.066,86.591H30.536    C32.896,204.752,43.778,170.172,62.229,139.585z M60.594,369.638c-17.455-29.899-27.769-63.481-30.059-98.623h83.146    c0.982,29.329,4.674,57.731,10.858,84.186C101.085,359.003,79.494,363.85,60.594,369.638z M96.216,415.784    c-6.38-6.381-12.322-13.081-17.831-20.055c16.323-4.526,34.571-8.359,54.214-11.433c5.53,17.103,12.194,33.105,19.928,47.662    c7.17,13.493,15.053,25.349,23.51,35.505C146.427,456.28,119.268,438.834,96.216,415.784z M240.984,478.115    c-22.808-6.389-44.384-27.217-61.936-60.249c-6.139-11.552-11.531-24.155-16.15-37.587c24.73-2.722,51.045-4.331,78.086-4.709    V478.115z M240.984,345.537c-29.988,0.409-59.217,2.292-86.59,5.507c-6.038-24.961-9.671-51.978-10.668-80.028h97.259V345.537z     M240.984,240.984h-97.315c0.911-28.834,4.602-56.605,10.828-82.201c27.198,3.4,56.366,5.468,86.487,6.06V240.984z     M240.984,134.808c-27.146-0.547-53.403-2.317-77.958-5.205c4.591-13.292,9.941-25.768,16.022-37.215    c17.551-33.032,39.128-53.86,61.936-60.249V134.808z M450.717,141.18c17.874,30.193,28.427,64.199,30.749,99.804h-83.088    c-0.889-29.844-4.584-58.749-10.85-85.647C410.661,151.601,431.984,146.848,450.717,141.18z M415.783,96.216    c6.029,6.029,11.661,12.349,16.914,18.91c-16.073,4.389-33.972,8.114-53.204,11.112c-5.548-17.208-12.243-33.305-20.02-47.941    c-6.579-12.382-13.758-23.391-21.43-32.969C366.845,56.535,393.273,73.705,415.783,96.216z M271.016,271.016h97.259    c-1.004,28.268-4.686,55.49-10.81,80.612c-27.194-3.381-56.349-5.43-86.449-6.006V271.016z M271.016,240.984v-76.041    c30.005-0.394,59.257-2.261,86.656-5.464c6.125,25.403,9.756,52.932,10.659,81.505H271.016z M271.014,32.139h0.001    c22.808,6.389,44.384,27.217,61.936,60.249c6.178,11.627,11.601,24.318,16.24,37.848c-24.763,2.712-51.108,4.309-78.177,4.674    V32.139z M271.016,478.115V375.657c27.12,0.532,53.357,2.286,77.903,5.156c-4.579,13.232-9.911,25.654-15.967,37.053    C315.4,450.898,293.824,471.726,271.016,478.115z M415.783,415.784c-23.051,23.051-50.21,40.496-79.821,51.678    c8.457-10.156,16.34-22.011,23.51-35.504c7.62-14.341,14.198-30.088,19.68-46.906c19.465,3.213,37.473,7.186,53.515,11.859    C427.424,403.457,421.801,409.765,415.783,415.784z M450.606,371.009c-18.635-5.991-40-11.032-63.326-15.01    c6.296-26.68,10.048-55.36,11.041-84.983h83.146C479.139,306.694,468.549,340.769,450.606,371.009z"></path>
                          </g>
                        </g>
                      </svg>
                    </span>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="u-expanded-width u-list u-list-2">
          <div className="u-repeater u-repeater-2">
            <div className="u-container-style u-list-item u-repeater-item">
              <div className="u-container-layout u-similar-container u-container-layout-2">
                <img
                  className="u-image u-image-round u-preserve-proportions u-radius-10 u-image-2"
                  src="https://womenrise.art/images/Sodi.png"
                  alt=""
                  data-image-width={1080}
                  data-image-height={1080}
                />
                <h3 className="u-align-center u-custom-font u-text u-text-font u-text-5">
                  Sodi
                </h3>
                <div className="u-social-icons u-spacing-10 u-social-icons-2">
                  <a
                    className="u-social-url"
                    href="https://twitter.com/cryptosodi"
                    title="Twitter"
                    target="_blank"
                  >
                    <span className="u-icon u-icon-circle u-social-icon u-social-twitter u-text-grey-5 u-icon-5">
                      <svg
                        className="u-svg-link"
                        preserveAspectRatio="xMidYMin slice"
                        viewBox="0 0 112 112"
                        style={{}}
                      >
                        <use
                          xmlnsXlink="http://www.w3.org/1999/xlink"
                          xlinkHref="#svg-0807"
                        ></use>
                      </svg>
                      <svg
                        className="u-svg-content"
                        viewBox="0 0 112 112"
                        x="0px"
                        y="0px"
                        id="svg-0807"
                      >
                        <path d="M92.2,38.2c0,0.8,0,1.6,0,2.3c0,24.3-18.6,52.4-52.6,52.4c-10.6,0.1-20.2-2.9-28.5-8.2 c1.4,0.2,2.9,0.2,4.4,0.2c8.7,0,16.7-2.9,23-7.9c-8.1-0.2-14.9-5.5-17.3-12.8c1.1,0.2,2.4,0.2,3.4,0.2c1.6,0,3.3-0.2,4.8-0.7 c-8.4-1.6-14.9-9.2-14.9-18c0-0.2,0-0.2,0-0.2c2.5,1.4,5.4,2.2,8.4,2.3c-5-3.3-8.3-8.9-8.3-15.4c0-3.4,1-6.5,2.5-9.2 c9.1,11.1,22.7,18.5,38,19.2c-0.2-1.4-0.4-2.8-0.4-4.3c0.1-10,8.3-18.2,18.5-18.2c5.4,0,10.1,2.2,13.5,5.7c4.3-0.8,8.1-2.3,11.7-4.5 c-1.4,4.3-4.3,7.9-8.1,10.1c3.7-0.4,7.3-1.4,10.6-2.9C98.9,32.3,95.7,35.5,92.2,38.2z"></path>
                      </svg>
                    </span>
                  </a>
                </div>
                <h3 className="u-align-center u-custom-font u-text u-text-font u-text-6">
                  {" "}
                  Developer
                </h3>
                <h2 className="u-align-justify u-custom-font u-text u-text-font u-text-7">
                  {" "}
                  Sodi is the developer and a crypto enthusiast. He has over 5
                  years of experience in blockchain and over 10 years of game
                  development experience.&nbsp;
                  <br />
                  <br />
                  Fun fact about Sodi: He’s not your typical coder sitting on
                  his computer table sipping-on soda and playing video games...
                  just kidding, he totally is! But he’s an absolute bad-ass
                  coding ninja so he has that going for him.&nbsp;
                </h2>
              </div>
            </div>
            <div className="u-container-style u-list-item u-repeater-item">
              <div className="u-container-layout u-similar-container u-container-layout-3">
                <img
                  className="u-image u-image-round u-preserve-proportions u-radius-10 u-image-3"
                  src="https://womenrise.art/images/Azkari.png"
                  alt=""
                  data-image-width={1080}
                  data-image-height={1080}
                />
                <h3 className="u-align-center u-custom-font u-text u-text-font u-text-8">
                  {" "}
                  Aski
                </h3>
                <div className="u-social-icons u-spacing-10 u-social-icons-3">
                  <a
                    className="u-social-url"
                    href="https://twitter.com/Aski1787"
                    title="Twitter"
                    target="_blank"
                  >
                    <span className="u-icon u-icon-circle u-social-icon u-social-twitter u-text-grey-5 u-icon-6">
                      <svg
                        className="u-svg-link"
                        preserveAspectRatio="xMidYMin slice"
                        viewBox="0 0 112 112"
                        style={{}}
                      >
                        <use
                          xmlnsXlink="http://www.w3.org/1999/xlink"
                          xlinkHref="#svg-04a5"
                        ></use>
                      </svg>
                      <svg
                        className="u-svg-content"
                        viewBox="0 0 112 112"
                        x="0px"
                        y="0px"
                        id="svg-04a5"
                      >
                        <path d="M92.2,38.2c0,0.8,0,1.6,0,2.3c0,24.3-18.6,52.4-52.6,52.4c-10.6,0.1-20.2-2.9-28.5-8.2 c1.4,0.2,2.9,0.2,4.4,0.2c8.7,0,16.7-2.9,23-7.9c-8.1-0.2-14.9-5.5-17.3-12.8c1.1,0.2,2.4,0.2,3.4,0.2c1.6,0,3.3-0.2,4.8-0.7 c-8.4-1.6-14.9-9.2-14.9-18c0-0.2,0-0.2,0-0.2c2.5,1.4,5.4,2.2,8.4,2.3c-5-3.3-8.3-8.9-8.3-15.4c0-3.4,1-6.5,2.5-9.2 c9.1,11.1,22.7,18.5,38,19.2c-0.2-1.4-0.4-2.8-0.4-4.3c0.1-10,8.3-18.2,18.5-18.2c5.4,0,10.1,2.2,13.5,5.7c4.3-0.8,8.1-2.3,11.7-4.5 c-1.4,4.3-4.3,7.9-8.1,10.1c3.7-0.4,7.3-1.4,10.6-2.9C98.9,32.3,95.7,35.5,92.2,38.2z"></path>
                      </svg>
                    </span>
                  </a>
                </div>
                <h3 className="u-align-center u-custom-font u-text u-text-font u-text-9">
                  {" "}
                  Project Director
                </h3>
                <h2 className="u-align-justify u-custom-font u-text u-text-font u-text-10">
                  {" "}
                  Aski is the project director and the brand strategist with
                  over 7 years experience in business consulting. He has
                  background in chartered accounting and has worked for big 4
                  accounting firms as a risk consultant.&nbsp;
                  <br />
                  <br />
                  Fun fact about Aski: He’s a total space-nerd and a movie buff.
                  Watching back to back movies at a cinema is a typical weekend
                  for him... Aski is also married to Maliha and is her biggest
                  supporter! And yes, that does mean she’s being dragged to
                  cinema every weekend!!!
                </h2>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
